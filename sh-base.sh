# Choose the first available executable
function choose_bin() {
  local cmd
  ...
  echo "No available executables found: $@" >&2

  return 1
}

# Choose the first available command (executable + args)
function choose_cmd() {
  local full_cmd bin bins=()
  ...
  echo "No available executables found: ${bins[@]}" >&2
...
# Set DEFAULT_BROWSER on your local machine
www() {
  if [[ -z "$DEFAULT_BROWSER" ]]; then
    echo "Set DEFAULT_BROWSER to e.g. 'open -a safari' or 'firefox-developer-edition'" >&2
    return 1
  fi

  sh -c "$DEFAULT_BROWSER $*"
}
...
function wttr() {
  ...
  cmd=$(choose_cmd curl "wget -qO-" "http -b")
